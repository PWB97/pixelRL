...
def final_test(loader, agent, fout):
    ...
    for i in range(TEST_NUM):
        ...
        cv2.imwrite('./resultimage/'+str(i)+'_moving.png', m)
        f = np.maximum(0, fixed[i][0])
        f = (f * 255).astype(np.uint8)
        cv2.imwrite('./resultimage/' + str(i) + '_fixed.png', f)
...
def main(fout):
    ...
    chainer.serializers.load_npz('./model/pixel-reg3000/model.npz', agent.model)
